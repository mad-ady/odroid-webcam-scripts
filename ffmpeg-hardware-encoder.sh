...
/usr/local/bin/c2cap -w $W -h $H -f $FRAMERATE -p yuyv -b $BANDWIDTH -d $VIDEO -o - |/usr/bin/ffmpeg -r $FRAMERATE -thread_queue_size 256 -f h264 -i - -thread_queue_size 32 -f pulse -server 127.0.0.1 -i alsa_input.usb-Sonix_Technology_Co.__Ltd._USB_2.0_Camera-02.analog-mono -acodec libmp3lame -async 1 -c:v copy -override_ffserver http://localhost:8099/mjpg-streamer.ffm
#sleep 30
